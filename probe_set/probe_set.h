...
struct vitalStatistics {
  vitalStatistics();
  vector<int> index;
  vector<string> statNames;          // seems wasteful to store within everything.. 
  vector< vector<float> > stats;     // the actual statistics.. the names are in the statNames vector.. hmm.  
  //float anovaScore;
  //float euclideanQuality;
  //float variationCoefficient;    // std/mean ..
};
...
struct nadFileInfo {
  ...
  int maxIndex;
  int chipIndex;
  int fileNo;                      // the number of files from which the file was made.. 
  vector<float> experimentIndices;
  vector<string> fileNames;      // the filenames from somewhere else.. 
...
struct sessionInformation {
  ...
  int index;       // the index given in the expression database. 
  int owner;       // the session owner. .. good to know
  string title;
  string description;
  vector<string> keywords;    // vector, cheaper than set.
  vector<int> otherMembers;   // other members.. of the set.
  
  sessionInformation();
  sessionInformation(int i, int o, string t, string d, vector<string> kw, vector<int> om);
};
...
struct annotationInformation {
  ...
  int owner;
  string annotation;
  vector<int> otherMembers;      // not really members, but the best word I could think of at the moment.
  
  annotationInformation(int i, int o, string a, vector<int> om);
...
struct blastMatch {
  ...
  blastMatch();
  blastMatch(string mI, int I, int al);
  string matchId;
  int length;
  int af_length;
...
struct blastGenomeMatch {
  ...
  blastGenomeMatch();
  blastGenomeMatch(string c, int l, int ls, int le, int lst, int m, int ms, int me, int mst, int h, int hs, int he, int hst, int g, int af, bool o);
  ...
  multimap<string, string> lowerD;
  multimap<string, string> midD;
  multimap<string, string> higherD;
...
struct probe_data {
  ...
  int ensemblBlastGuess;   // the dbIndex of the best (??) matched gene by blast.. 0 if no match.. 
  ...
  multimap<double, blastGenomeMatch> genomeMatches;  // where double is the expectation.. !
  vector<sessionInformation> sessions;
  vector<annotationInformation> annotations;        // user annotation and sessions. Includes links to other genes covered.. !! 
