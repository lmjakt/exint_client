...
vector<string> split_pat(string& line, string& pat){
...
probe_set::probe_set(int i, vector< vector<float> >& p, vector<int>& eindices){
  ...
  //e_quality = euclidean();  // anyway  I don't use this.
}
...
blastGenomeMatch::blastGenomeMatch(string c, int l, int ls, int le, int lst, int m, int ms, int me, int mst, int h, int hs, int he, int hst, int g, int af, bool o){
  chromosome = c;
  lowerIndex = l;
  lowerStart = ls;
  lowerEnd = le;
...
sessionInformation::sessionInformation(int i, int o, string t, string d, vector<string> kw, vector<int> om){
  index = i;
  owner = o;
  title = t;
  description = d;
...
annotationInformation::annotationInformation(int i, int o, string a, vector<int> om){
  index = i;
  owner = o;
  annotation = a;
  otherMembers = om;
