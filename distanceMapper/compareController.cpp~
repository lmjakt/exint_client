...
void CompareController::newDistances(exptDistanceInfo info){
  QString name;
  if(info.isFlat){
...
void CompareController::recordDistances(exptDistanceInfo& info){
...
void CompareController::compareFlat(){
  emit doFlatCompare(flatSigma->fvalue(), flatOrder->fvalue());
}
