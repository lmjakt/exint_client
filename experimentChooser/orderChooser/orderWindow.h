...
class OrderWindow: public QWidget
{
  Q_OBJECT
    public:
  //  OrderWindow(QWidget *parent=0, const char *name=0);
  OrderWindow(QWidget *parent=0, const char *name=0);
  ~OrderWindow();
  //  map<float, exptInfo>* eMap;       // stuff 

  OrderWidget* inputWindow;     // replace all the canvas stuff with this.. (should be private surely)
 ...
 signals:
  ...
  void setExptOrder(multimap<float, exptInfo>);
  void changeActivityStates(map<int, bool>);    // sets whether or not to show the expression.. 
